...
expro <- inner_join(genes,expro, by = "gene")
...
write.table(outTab,file="output/uniCoxResult.txt",sep="\t",row.names=F,quote=F)

sigTab=outTab[as.numeric(as.vector(outTab$pvalue))<0.05,] #P值的筛选阈值可设为0.10
write.table(sigTab,file="output/uniCoxResult.Sig.txt",sep="\t",row.names=F,quote=F)
