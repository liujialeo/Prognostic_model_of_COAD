...
dataclinical <- dataclinical[-c(502:503),]
rownames(dataclinical) <- dataclinical[,1]

rt=read.table("output/risk.txt",header=T,sep="\t") ##读入多因素COX分析结果，将riskScore作为一种临床信息
...
write.table(cbind(id=rownames(cbind(rt[,1:2],risk_score,risk_level)),cbind(rt[,1:2],risk_score,risk_level)),"output/risk_score.txt",sep="\t",quote=F,row.names=F)

#绘制ROC曲线
rt=read.table("output/risk_score.txt",header=T,sep="\t",check.names=F,row.names=1)
#1年ROC
pdf(file="output/nomogram_ROC-1.pdf",width=6,height=6)
...
#3年ROC
pdf(file="output/nomogram_ROC-3.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
roc=survivalROC(Stime=rt$futime, status=rt$fustat, marker = rt$risk_score, 
...
#5年ROC
pdf(file="output/nomogram_ROC-5.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
roc=survivalROC(Stime=rt$futime, status=rt$fustat, marker = rt$risk_score, 
...
#整合1，3，5年ROC
pdf(file="output/nomogram_ROC.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
...
legend("bottomright", 
       c("1-year AUC:0.801","3-year AUC:0.763","5-year AUC:0.812"),
       lwd=2,
