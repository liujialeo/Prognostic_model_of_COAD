...
### 1.准备细胞marker
cellMarker <- data.table::fread("resource/cellMarker.csv",data.table = F)
...
save(cellMarker,file = "resource/cellMarker_ssGSEA.Rdata")
#########################################
...
ggsave(filename = "output/immnue_scRNA_UCN.pdf")
##GABRD
ggplot(data =dd1, aes(x = celltype, y = NES))+
  ...
  scale_fill_manual(values= colset[c(1,2)] )
ggsave(filename = "output/immnue_scRNA_GABRD.pdf")
