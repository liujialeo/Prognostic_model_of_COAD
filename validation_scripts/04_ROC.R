...
rm(list = ls())
library(survivalROC)
rt=read.table("test_output/risk.txt",header=T,sep="\t",check.names=F,row.names=1)

#1年ROC
pdf(file="test_output/ROC-1.pdf",width=6,height=6)
...
#3年ROC
pdf(file="test_output/ROC-3.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
roc=survivalROC(Stime=rt$futime, status=rt$fustat, marker = rt$riskScore, 
...
#5年ROC
pdf(file="test_output/ROC-5.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
roc=survivalROC(Stime=rt$futime, status=rt$fustat, marker = rt$riskScore, 
...
#整合1，3，5年ROC
pdf(file="test_output/ROC.pdf",width=6,height=6)
par(oma=c(0.5,1,0,1),font.lab=1.5,font.axis=1.5)
...
legend("bottomright", 
       c("1-year AUC:0.708","3-year AUC:0.727","5-year AUC:0.755"),
       lwd=2,
